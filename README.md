...
- `scripts/deploy.sh` builds the image for `linux/amd64` (the Coolify host's architecture), pushes to GHCR, and triggers a Coolify redeploy. Its secrets live only in `.deploy.env` (gitignored — never read by the running app):

  ```bash
  cp .deploy.env.example .deploy.env   # fill in GHCR + Coolify creds
