...
- **Stable tier numbering (optional).** "Maintain structure" numbers ring groups by tier priority (priority 2 → `<prefix>2`) and skips empty tiers instead of collapsing them, so numbering other FreePBX routes depend on stays consistent. With it, you can also **pin the entry group** (`<prefix>1`) so the group your inbound route hits is always live — when priority 1 has no members it becomes a member-less pass-through that forwards to the first active tier.
